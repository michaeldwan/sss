# SSS is a Ruby library for Amazon's Simple Storage Service's REST API (http://aws.amazon.com/s3).
...
#   # => true
# 
# The SSS library ships with an interactive shell called <tt>s3sh</tt>. From within it, you have access to all the operations the library exposes from the command line.
# 
#   % s3sh
#   >> Version
# 
...
#   fi
# 
# See more connection details at SSS::Connection::Management::ClassMethods.
module SSS
  constant :DEFAULT_HOST, 's3.amazonaws.com'
  
  # SSS::Base is the abstract super class of all classes who make requests against S3, such as the built in
  ...
  #
  #   SSS::Base.establish_connection!(:access_key_id => '...', :secret_access_key => '...')
  #
  ...
  class Base      
    class << self
      ...
      # 
      #   class JukeBoxSong < SSS::S3Object
      ...
      def set_current_bucket_to(name)
        raise ArgumentError, "`#{__method__}' must be called on a subclass of #{self.name}" if self == SSS::Base
        instance_eval(<<-EVAL)
          def current_bucket
            '#{name}'
          end
